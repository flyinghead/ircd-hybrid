 ...
 * \version $Id: client.c 5751 2015-04-02 17:27:13Z michael $
 */
...
exit_one_client(struct Client *source_p, const char *comment)
{
  ...
  /*FILE *f = fopen(USER_COUNT_FILENAME, "wt");
  if (f == NULL) {
	...
	fprintf(f, "%d", count);
        fprintf(f, "\n");
        //sendto_realops_flags(UMODE_ALL, L_ALL, SEND_NOTICE, "Connection count: %d", count);
  }
  fclose(f);
  */
